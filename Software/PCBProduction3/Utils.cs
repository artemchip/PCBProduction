...
namespace PCBProduction3
{
    class Utils
    {
        ...
        public static double StrToDouble(String str)
        {
            if (str.StartsWith("-"))
            {
                return -(double)(CustomParseDecimal(str.Substring(1)));
            } else
            {
        ...
        {
            if (axisFeedRateMmPerMin <= 0.0f || axisAccMmPerSecSq <= 0.0f)
            {
                throw new Exception("Неверные параметры MillisecToExecuteAxisMove.");
            }
